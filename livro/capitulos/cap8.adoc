...
++++
\[
\frac{\widehat{p}-p}{\sqrt{\widehat{p}(1-\widehat{p})/n}} \stackrel{aprox.}{\sim} N(0,1),
\]
++++
ou seja, se trocarmos x:[p(1-p)/n] por x:[\widehat{p}(1-\widehat{p})/n], o resultado ainda vale. Este
resultado será útil na construção de intervalos de confiança.
...
Suponha que temos um estimador para um certo parâmetro x:[\theta] dado por x:[\widehat{\theta}].
Além disso, suponha que temos a seguinte aproximação:
[latexmath]
++++
\[
\frac{\widehat{\theta}-\theta}{\sigma} \stackrel{aprox.}{\sim} N(0,1).
\]
++++

Queremos então utilizar x:[\widehat{\theta}] e a aproximação acima para construir um 
intervalo de confiança para x:[\theta], ou seja, queremos utilizar x:[\widehat{\theta}]
para construir um intervalo aleatório, do tipo x:[[T_1,T_2\]], onde x:[T_1] e x:[T_2]
dependem de x:[\widehat{\theta}] tal que 
[latexmath]
++++
\[
P(T_1< \theta \leq T_2)) \approx 1-\alpha,
\]
++++
onde x:[\alpha] é um nível de significância determinado previamente. Normalmente 
costuma-se escolher x:[\alpha = 0,01, \alpha = 0,05] ou x:[\alpha = 0,10], isto é,
estamos afirmando que em apenas em 1%, ou 5%, ou 10%, das amostras possíveis (de mesmo tamanho)
da população, o intervalo de confiança não contém o parâmetro x:[\theta]. Nestes casos,
dizemos que estamos construindo intervalos de confiança de níveis de confiança
de 99%, 95% ou 90%, respectivamente.

Observe que a aproximação acima obtida para x:[\widehat{\theta}] nos fornece:
[latexmath]
++++
\[
\begin{array}{lll}
P\Big(-C < \frac{\widehat{\theta}-\theta}{\sigma} \leq C\Big) &=& P\Big( \frac{\widehat{\theta}-\theta}{\sigma} \leq C\Big) - P\Big(\frac{\widehat{\theta}-\theta}{\sigma}\leq -C\Big)\\
&\approx& \Phi(C) - \Phi(-C)\\
&=& \Phi(C) - 1 + \Phi(C)\\
&=& 2\Phi(C) -1.
\end{array}
...
\[
\begin{array}{lll}
P\Big(-C < \frac{\widehat{\theta}-\theta}{\sigma} \leq C\Big) &=& P\Big( -C\sigma < \widehat{\theta}-\theta \leq C\sigma\Big)\\
&=&P\Big( -C\sigma - \widehat{\theta} < -\theta \leq C\sigma - \widehat{\theta}\Big)\\
&=& P\Big( -C\sigma + \widehat{\theta} \leq \theta < C\sigma + \widehat{\theta}\Big).
\end{array}
\]
++++
...
++++
\[
P\Big( -C\sigma + \widehat{\theta} \leq \theta < C\sigma + \widehat{\theta}\Big)\approx 2\Phi(C) -1.
\]
++++

Se quisermos um nível x:[1-\alpha], temos que resolver x:[2\Phi(C) -1 = 1-\alpha] o que fornece
[latexmath]
++++
\[
C = \Phi^{-1}\Big(1-\frac{\alpha}{2}\Big),
\]
++++
onde x:[\Phi^{-1}(1-\alpha/2)] é o valor encontrado na tabela da normal, tal que a probabilidade
de ser menor ou igual a este valor é de x:[1-\alpha/2].

Finalmente, obtemos que um intervalo de confiança de nível x:[\alpha] para x:[\theta] é dado por
[latexmath]
++++
\[
\big[ -C\sigma + \widehat{\theta}; C\sigma + \widehat{\theta}\big],
\]
++++
onde x:[C] é dado por x:[C = \Phi^{-1}\Big(1-\frac{\alpha}{2}\Big)]. 

==== Intervalo de Confiança para a Média

(((Intervalo de Confiança, para a Média)))

Seja x:[X_1,\ldots,X_n] uma AAS de uma variável aleatória comum x:[X] satisfazendo x:[E(X_i) = \mu] 
e x:[Var(X_i)=\sigma^2]. Então, seja x:[\overline{X}] a média dessa AAS:
[latexmath]
++++
\[
\overline{X} = \frac{1}{n} \sum_{i=1}^n X_i.
...
Pelo que vimos na subseção anterior, isto nos diz que um intervalo de confiança de nível x:[1-\alpha] para
a média é dado por
[latexmath]
++++
\[
\Big[ -C \frac{\sigma}{\sqrt{n}} + \overline{X}; C \frac{\sigma}{\sqrt{n}} + \overline{X} \Big],
\]
++++
onde x:[C] é dado por x:[C = \Phi^{-1}\Big(1-\frac{\alpha}{2}\Big)].

.Exemplo de cálculo de intervalo de confiança para a média
====
Suponha que as alturas dos alunos da UFPB tenham distribuição normal com  
x:[\sigma = 15]cm. Foi retirada uma amostra aleatória de 100 alunos obtendo-se
x:[\overline{X}=175]cm. Vamos construir um intervalo de 95\% de confiança para 
a verdadeira altura média dos alunos.

Primeiramente, note que estamos querendo x:[1-\alpha = 0,95], o que nos fornece
x:[\alpha = 0,05] e desta forma, x:[1-\alpha/2 = 0,975].

Olhando para a tabela da normal, vemos que x:[C = \Phi^{-1}(1-\alpha/2)] é dado
por 1,96.
...
++++
\[
\Big[ -1,96\cdot \frac{15}{\sqrt{100}} + 175; 1,96\cdot \frac{15}{\sqrt{100}} + 175\Big].
\]
++++
Realizando a conta, obtemos que o intervalo, ao nível de 95% de confiança para o verdadeiro
valor da altura média dos alunos da UFPB é 
[latexmath]
++++
\[
\Big[ -1,72; 1,78\Big].
...
Seja x:[X_1,\ldots,X_n] uma AAS de uma variável aleatória Bernoulli x:[X] com parâmetro x:[p],
isto é, as variáveis x:[X_i] assumem o valor 1 com probabilidade x:[p], e 0 com probabilidade
x:[1-p]. Seja x:[\widehat{p}] a proporção da amostra que assume valor 1 (ou em exemplos práticos
a proporção da amostra que satisfaz uma determinada condição), então, temos que x:[\widehat{p}]
é dado por
[latexmath]
++++
\[
\widehat{p} = \overline{X} = \frac{1}{n} \sum_{i=1}^n X_i.
\]
++++
...
Desta forma, utilizando o que vimos na construção de intervalos de confiança,
um intervalo de confiança de nível x:[1-\alpha] para a proporção é dado por
[latexmath]
++++
\[
\Big[ -C \frac{\sqrt{\widehat{p}(1-\widehat{p})}}{\sqrt{n}} + \widehat{p}; C \frac{\sqrt{\widehat{p}(1-\widehat{p})}}{\sqrt{n}} + \widehat{p} \Big],
\]
++++
onde x:[C] é dado por x:[C = \Phi^{-1}\Big(1-\frac{\alpha}{2}\Big)].

.Exemplo de cálculo de intervalo de confiança para a proporção
====
Uma amostra de 300 habitantes de uma cidade mostrou que 180 desejavam a água fluorada. 
Vamos encontrar o intervalo de confiança de 95% para a população favorável a fluoração.
...
++++
\[
\Big[ -1,96\cdot \frac{\sqrt{0,6\cdot 0,4}}{\sqrt{300}} + 0,6; 1,96\cdot \frac{\sqrt{0,6\cdot 0,4}}{\sqrt{300}} + 0,6\Big].
\]
++++
Realizando a conta, obtemos que o intervalo, ao nível de 95% de confiança para o verdadeiro
valor da proporção da população favorável a fluoração é
[latexmath]
++++
\[
\Big[ 0,54; 0,65\Big].
...
Considere duas AAS independentes de tamanhos x:[n_1] e x:[n_2] das duas populações. Vimos que
a distribuição amostral da diferença x:[(\overline{X}_1-\overline{X}_2)],
para x:[n_1] e x:[n_2] suficientemente grandes, satisfaz
[latexmath]
++++
\[
\frac{(\overline{X}_1-\overline{X}_2)-(\mu_1-\mu_2)}{\sqrt{\sigma_1^2/n_1+\sigma_2^2/n_2}}  \stackrel{aprox.}{\sim} N(0,1).
\]
++++


Desta forma, utilizando o que vimos na construção de intervalos de confiança,
um intervalo de confiança de nível x:[1-\alpha] para a diferença de médias é dado por
[latexmath]
++++
\[
\Big[ -C \sqrt{\sigma_1^2/n_1+\sigma_2^2/n_2} + \overline{X}_1-\overline{X}_2; C \sqrt{\sigma_1^2/n_1+\sigma_2^2/n_2} + \overline{X}_1-\overline{X}_2 \Big],
\]
++++
onde x:[C] é dado por x:[C = \Phi^{-1}\Big(1-\frac{\alpha}{2}\Big)].

.Exemplo de cálculo de intervalo de confiança para a diferença de médias
====
Um teste psicológico destinado a medir a precisão com que uma pessoa julga outras pessoas, foi realizado. 
As notas possíveis do teste variam de 0 a 41. Durante sua elaboração o teste foi aplicado a vários grupos 
com diferentes de pessoas. De acordo com os resultados observados, vamos construir um intervalo de confiança
para a diferença entre as médias dos grupos de homens e de mulheres, com $95\%$ de confiança.
...
++++
\[
\Big[ -1,96\cdot \sqrt{\frac{(5,05)^2}{133}+\frac{(5,44)^2}{162}}  + 25,34-24,94; 1,96\cdot \sqrt{\frac{(5,05)^2}{133}+\frac{(5,44)^2}{162}}  + 25,34-24,94\Big].
\]
++++
Realizando a conta, obtemos que o intervalo, ao nível de 95% de confiança para o verdadeiro
valor da diferença entre as médias dos grupos de homens e de mulheres é
[latexmath]
++++
\[
\Big[ -0,80; 1,60\Big].
