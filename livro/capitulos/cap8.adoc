...
++++
\[
P(T_1< \theta \leq T_2)) \approx \alpha,
\]
++++
onde x:[\alpha] é um nível de significância determinado previamente. Normalmente 
costuma-se escolher x:[\alpha = 0,99, \alpha = 0,95] ou x:[\alpha = 0,90], isto é,
níveis de 99%, 95% ou 90%, respectivamente.
...
Se quisermos um nível x:[\alpha], temos que resolver x:[2\Phi(C) -1 = \alpha] o que fornece
[latexmath]
++++
\[
C = \Phi^{-1}\Big(\frac{1+\alpha}{2}\Big),
\]
++++
...
\]
++++
onde x:[C] é dado por x:[C = \Phi^{-1}\Big(\frac{1+\alpha}{2}\Big)]. 
